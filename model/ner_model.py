...
from transformers import get_linear_schedule_with_warmup, AutoModel, XLMRobertaForTokenClassification

from log import logger
from utils.metric import SpanF1
from utils.reader_utils import extract_spans, get_tags
from adapters import AdapterConfig, AutoAdapterModel 
from adapters import BnConfig,SeqBnInvConfig,LoRAConfig
...
class NERBaseAnnotator(pl.LightningModule):
    def __init__(self,
        ...
        self.encoder = AutoAdapterModel.from_pretrained(encoder_model, return_dict=True)
        
        if self.stage=='fit':
            if adapter and adapter_file:
                adapter_name=self.load_from_adapter(adapter_file,head=True)
                self.encoder.train_adapter(adapter_name)
            elif adapter and not adapter_file:
                # adapter_name="bottleneck_adapter"
                # self.encoder.add_tagging_head(adapter_name,num_labels=self.target_size)
                # config = AdapterConfig(mh_adapter=True, output_adapter=True, reduction_factor=16, non_linearity="relu")
                # self.encoder.add_adapter(adapter_name, config=config)
                # self.encoder.set_active_adapters(adapter_name)
                # self.encoder.train_adapter(adapter_name)
                # adapter_name="lang_adapter"
                # self.encoder.add_tagging_head(adapter_name,num_labels=self.target_size)
                ...
                self.encoder.add_tagging_head("bottleneck_adapter",num_labels=self.target_size)
        ...
        if adapter and adapter_file:
            self.save_hyperparameters('pad_token_id', 'encoder_model','lr','dropout_rate','batch_size', 'adapter_file')
        else:
            self.save_hyperparameters('pad_token_id', 'encoder_model','lr','dropout_rate','batch_size')
        

    def setup_model(self, stage_name):
        if (stage_name == 'fit' or stage_name=='fine_tune') and self.train_data is not None:
    ...
    def configure_optimizers(self):
        ...
        if self.stage == 'fit' or self.stage == 'fine_tune':
    ...
    def perform_forward_step(self, batch, mode=''):
        ...
        token_scores=embedded_text_input.logits
    ...
    def load_from_adapter(self,adapter_file, head=True):
        adapter_name=self.encoder.load_adapter(adapter_file,with_head=head)
        self.encoder.set_active_adapters(adapter_name)
